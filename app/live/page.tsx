...
const shiftActions=["Shift In","Shift Out"];
...
export default function LiveAttendance(){
 ...
 async function punch(employee:Employee,action:string){
   ...
   const today=punches.filter(p=>p.employee_id===employee.id&&p.punched_at.slice(0,10)===selectedDate).sort((a,b)=>new Date(b.punched_at).getTime()-new Date(a.punched_at).getTime());
   const latestShiftPunch=today.find(p=>p.action==="Shift In"||p.action==="Shift Out");
   const open=latestShiftPunch?.action==="Shift In";
   if(action==="Shift In"&&open){setMessage("Employee is already checked in.");return}
   if(action==="Shift Out"&&!open){setMessage("Employee is not checked in.");return}
   ...
   setLoading(false);if(error)setMessage(error.message);else {setMessage(action+" recorded for "+employee.full_name);await loadPunches();}
 }
