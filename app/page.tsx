...
function DashboardPage({ stats, employees, punches, rules, date, onDate, onAdd, onGo }: { stats: { present: number; absent: number; break: number; notIn: number; late: number }; employees: Employee[]; punches: Punch[]; rules: Rules; date: string; onDate: (date: string) => void; onAdd: () => void; onGo: (label: string) => void }) {
  ...
  return <>
    ...
    <section className="dashboard-grid"><div className="panel attendance-panel"><div className="panel-header"><div><h3>Attendance</h3><p>Selected date · {date}</p></div><button className="ghost-button" onClick={() => onGo("Attendance")}>View all →</button></div><div className="table-wrap"><table><thead><tr><th>EMPLOYEE</th><th>LOCATION</th><th>SHIFT IN</th><th>STATUS</th></tr></thead><tbody>{employees.slice(0, 7).map((person) => <tr key={person.id}><td><div className="employee-cell"><div className="avatar small">{person.avatar}</div><div><strong>{person.name}</strong><small>{person.id}</small></div></div></td><td>{person.location}</td><td>{person.shiftIn}</td><td><StatusBadge status={person.late ? "Late" : person.status} /></td></tr>)}</tbody></table></div></div><div className="panel"><div className="panel-header"><div><h3>Recent Activity</h3><p>Selected date · face verification events</p></div></div><div className="activity-list">{punches.slice(0, 6).map((punch) => <div className="activity" key={punch.id}><div className="activity-dot" /><div className="activity-body"><strong>{punch.employee}</strong><span>{punch.action} · {punch.faceVerified ? "Face verified" : "Web admin"}</span></div><time>{punch.time}</time></div>)}</div></div></section>
...
function UsersPage() {
  ...
  const [users, setUsers] = useState([{ name: "Raju", email: "admin@mahamart.com", role: "Company Admin", location: "All Locations", active: true }, { name: "Store Manager", email: "manager@mahamart.com", role: "Store Manager", location: "Head Office", active: true }]);
  ...
  function addUser(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const name = String(form.get("name") || "").trim(); const email = String(form.get("email") || "").trim(); if (!name || !email) return; setUsers((current) => [...current, { name, email, role: String(form.get("role") || "Supervisor"), location: String(form.get("location") || "Head Office"), active: true }]); setShowUserForm(false); }
  return <PageFrame title="Users & Roles" subtitle="Control access, roles and location scope" action={<button className="primary-button" onClick={() => setShowUserForm(true)}>+ Add User</button>}><div className="role-layout"><div className="panel"><div className="panel-header"><div><h3>System Users</h3><p>Admin accounts that can access the web app</p></div></div><div className="table-wrap"><table><thead><tr><th>USER</th><th>EMAIL</th><th>ROLE</th><th>LOCATION SCOPE</th><th>STATUS</th></tr></thead><tbody>{users.map((user) => <tr key={user.email}><td><strong>{user.name}</strong></td><td>{user.email}</td><td><span className="role-pill">{user.role}</span></td><td>{user.location}</td><td><StatusBadge status={user.active ? "Present" : "Absent"} /></td></tr>)}</tbody></table></div></div><div className="settings-card"><h3>Role permissions</h3><p className="card-description">Permission matrix for the planned backend.</p><label className="field-label">Select role<select className="role-select" value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)}>{roles.map((r) => <option key={r}>{r}</option>)}</select></label><div className="permission-list">{permissions.map((permission) => <div className="permission-row" key={permission}><span>{permission}</span><span className="permission-on">{selectedRole === "Employee" && ["Manage Employees","Edit Attendance","Export Reports","Manage Locations","Manage Users","Manage Rules"].includes(permission) ? "No" : "Yes"}</span></div>)}</div></div></div>{showUserForm && <UserForm roles={roles} onClose={() => setShowUserForm(false)} onSubmit={addUser} />}</PageFrame>;
}
...
function UserForm({ roles, onClose, onSubmit }: { roles: string[]; onClose: () => void; onSubmit: (e: FormEvent<HTMLFormElement>) => void }) {
